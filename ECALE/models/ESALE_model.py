...
def get_mask(source_ids, target_ids, padding_id, with_ulm):
    ...
    mask00 = source_ids.ne(padding_id)[:, None, :]*source_ids.ne(padding_id)[:, :, None]
    mask01 = torch.zeros((batch_size, code_len, comment_len), dtype=torch.uint8).to(source_ids.device)
    mask0 = torch.cat((mask00, mask01), 2)
    mask10 = source_ids.ne(padding_id)[:, None, :]*target_ids.ne(padding_id)[:, :, None]
    if with_ulm:
        mask11 = torch.tril(target_ids.ne(padding_id)[:, None, :]*target_ids.ne(padding_id)[:, :, None], 0)
    else:
        mask11 = target_ids.ne(padding_id)[:, None, :]*target_ids.ne(padding_id)[:, :, None]
...
class ESALE(nn.Module):
    def __init__(self,
        ...
        ulmBias = nn.Parameter(torch.zeros(vocab_size))
        self.ulmDecoder.bias = ulmBias
